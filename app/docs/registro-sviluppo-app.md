...
| Il registratore manda l'**Invio anche quando il comando di regia è fallito** | `tools/registra-terminale.py`, `seconda_finestra` | **aperto, trovato dalla review di `codex` del 4 settembre 2026 e misurato**: con un comando annunciato che esce con 7, la scena riparte lo stesso e il registratore esce **0**. È esattamente il guasto che [`RegiaCompose`](../src/mongolab/infrastructure/regia.py) evita alzando `ComandoFallito` — «un `stop` fallito lascerebbe il primario in piedi, e i due numeri finali sarebbero zero millisecondi di interruzione e zero scritture perse» —, reintrodotto nel percorso della registrazione |
| Un'eccezione **dentro** il generatore dell'avanzamento non ferma il processo del dump | `infrastructure/backup.py`, `_avanzamento`: `kill()` sta solo nell'`except GeneratorExit` | **aperto, trovato dalla review di `codex` del 4 settembre 2026 e misurato, con una precisazione**: se l'interruzione arriva **solo al processo Python** il figlio resta vivo davvero (stato `S`); se arriva al **gruppo di processi**, cioè il Ctrl-C vero da terminale, il figlio muore da sé e resta uno zombie non raccolto, che il sistema riprende all'uscita. Il caso grave è quindi più stretto di così, ma esiste, e comprende il limite già dichiarato di `docker exec` |
