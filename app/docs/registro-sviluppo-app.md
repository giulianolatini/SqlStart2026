...
| Il carico non sapeva chiedere un write concern diverso dal predefinito | [V-083, riserve](../../docs/Sources.md#v-083) e [ADR-0112, conseguenze](../../docs/Decision.md#adr-0112) | **chiuso al Task 18** da [ADR-0114](../../docs/Decision.md#adr-0114). La riga era scritta male: diceva «la corsa con `w: 1` non è eseguibile», mentre `w` è un'opzione della stringa di connessione come `journal` ([S-076](../../docs/Sources.md#s-076)) e `connetti(**extra)` la portava al client dal Task 5. Mancava la parola sulla riga di comando, non il meccanismo. Aggiunto `--write-concern`, la misura è [V-088](../../docs/Sources.md#v-088): 18 913 µs contro 644 |
| Il fattore 1,38 della maggioranza viene da una corsa sola per configurazione | [V-088, riserve](../../docs/Sources.md#v-088) | aperto al **Task 18**. Le tre corse cambiano una cosa per volta, ma non sono ripetute; la variabilità fra corse identiche misurata in [V-083](../../docs/Sources.md#v-083) è del 20 % sulla resa, cioè dello stesso ordine del più piccolo dei tre fattori. Il 2,24× del giornale e il 29× sul cronometro del server sono fuori portata dal rumore; quello della maggioranza no |
| ~~`--tetto` ferma il carico, **non la scena**: un `mongodump` piantato tiene appesa `ScenarioBackup.esegui()` all'infinito~~ | [ADR-0118](../../docs/Decision.md#adr-0118) e [M-059](Sources.md#m-059) | **chiuso il 4 settembre 2026**, dentro la PR #5 e su indicazione del PO, che ha voluto il tetto come «una guardia oltre la quale non si può andare». Il numero viaggia fino alla porta — `BackupTool.dump(destinazione, *, tetto_s=None)` — e `SubprocessBackup` lo fa rispettare con un `threading.Timer` che abbatte il figlio, alzando `DumpTroppoLungo`. La misura che ha deciso la forma è la seconda: un thread di guardia che chiuda l'iteratore alza `ValueError: generator already executing`, perché il consumatore è **dentro** il frame del generatore, fermo sul tubo. Prima: 15 s contro un tetto di 0,05. Dopo: 2,07 s contro un tetto di 2 |
| ~~Se la fase dell'elezione solleva, il nodo **resta fermo o sospeso**: `ripara()` non sta in un `finally`~~ | [ADR-0119](../../docs/Decision.md#adr-0119) | **chiuso il 4 settembre 2026**, dentro la PR #5. Il PO ha approvato esplicitamente il prezzo che il rilievo segnalava: «l'attrezzo ti fa una domanda invece di uscire, per me è accettabile». Il `try` si apre **dopo** `rompe` — il `finally` copre ciò che è stato rotto, non ciò che non si è riusciti a rompere — e l'annuncio della ripresa resta dentro il `try`, così l'interruzione non paga una seconda pausa di `--step`. In più `_gia_fatto` assorbe l'`EOFError`: un'eccezione sollevata dentro un `finally` prende il posto di quella che passava, e senza terminale `input()` alza subito |
| `restore` non ha un tetto, e le due firme della porta non sono più simmetriche | [ADR-0118, conseguenze](../../docs/Decision.md#adr-0118) | **aperto e dichiarato** dal 4 settembre 2026. Darglielo vorrebbe dire un `--tetto` su `demo restore`, cioè superficie di riga di comando che nessuno ha chiesto; e la scena del restore non ha un carico accanto da fermare. Resta però un `mongorestore` che si pianta e tiene appesa la scena, cioè lo stesso difetto che il tetto ha appena chiuso sul dump |
| ~~Il registratore manda l'**Invio anche quando il comando di regia è fallito**~~ | `tools/registra-terminale.py`, `seconda_finestra` | **chiuso il 4 settembre 2026, lo stesso giorno del rilievo**, da [ADR-0117](../../docs/Decision.md#adr-0117): l'Invio parte solo se il comando è uscito con zero, altrimenti la scena viene abbattuta con un `SIGTERM` al gruppo e lo strumento esce con **125**. La prova che lo difende annuncia un comando che esce con 7 e pretende tre cose insieme: codice 125, `ripartito` assente dal `.cast`, il motivo su `stderr`. La quarta cosa che controlla — «uscita 7» su `stderr` — la passava **già** il codice rotto, ed è misurato: il registratore lo diceva e usciva 0 lo stesso. È la ragione per cui una riga di diagnostica non bastava |
| ~~Un'eccezione **dentro** il generatore dell'avanzamento non ferma il processo del dump~~ | `infrastructure/backup.py`, `_avanzamento`: `kill()` stava solo nell'`except GeneratorExit` | **chiuso il 4 settembre 2026, lo stesso giorno del rilievo**: l'`except` prende `BaseException`, che comprende `GeneratorExit` e tutto il resto. Da qui il figlio non si distingue — un `close()`, un `KeyboardInterrupt`, un errore rimandato dentro dal consumatore lasciano lo stesso `mongodump` a girare — e quindi non si distingue neanche il rimedio. La prova rossa teneva il figlio vivo per dieci secondi; verde, muore in mezzo. Resta il limite già dichiarato nel docstring: con `docker exec` muore il client, non lo strumento dentro il container |
