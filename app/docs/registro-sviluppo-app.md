...
| `--tetto` ferma il carico, **non la scena**: un `mongodump` piantato tiene appesa `ScenarioBackup.esegui()` all'infinito | [ADR-0101](../../docs/Decision.md#adr-0101) e [15](15-il-backup-a-caldo-e-la-finestra-che-si-misura.md) dicono che il tetto fa «terminare la scena anche se il dump non torna più» | **aperto, trovato dalla review di `codex` del 4 settembre 2026 e misurato**: con `tetto_s = 0,05 s` e un iteratore che non torna, la scena era ancora ferma dopo 15 s, cioè 300 volte il tetto. Il limite arriva solo a `WorkloadRunner`; il thread principale resta dentro `for avanzamento in self._strumento.dump(...)`. La documentazione promette più di ciò che il codice fa |
| Se la fase dell'elezione solleva, il nodo **resta fermo o sospeso**: `ripara()` non sta in un `finally` | `application/scenari.py`, `ScenarioFailover.esegui` | **aperto, trovato dalla review di `codex` del 4 settembre 2026 e misurato**: con un carico che solleva alla seconda fase, la regia riceve solo `('ferma', 'mongo-rs-1')` — mai `riavvia`. Vale per tutti e due i modi. La correzione non è ovvia quanto sembra: dentro la rete Compose la regia è `RegiaAnnunciata`, e un `finally` che annuncia la riparazione si fermerebbe su un `input()` proprio mentre qualcuno sta interrompendo la scena. Da portare al PO |
