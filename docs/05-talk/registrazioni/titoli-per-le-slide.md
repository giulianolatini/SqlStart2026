...
Dodici filmati muti, da mettere dentro le slide. Sono di due tagli:

- **sei montaggi** (`01`…`06`), che sono le scene del talk: uno o più `.cast` uno dietro l'altro,
  pensati per stare su una slide da soli;
- **sei scene singole** (`scena-03`…`scena-09`), utili se una slide deve mostrare **un pezzo solo**
...
con quella di chi parla. Undici su dodici sono fabbricati dalle registrazioni di terminale
con `make filmati` e si rifanno da capo in meno di un minuto; il quarto è girato dal vivo.

> **Le scene 2 e 4 non hanno un file a parte, ed è voluto.** Una «scena singola» serve a prendere
> un pezzo di un montaggio: ma i montaggi `01` e `03` contengono **una scena sola**, quindi il
> pezzo e l'intero sarebbero stati lo stesso file byte per byte. Se una slide ti chiede solo la
> scena 2, usa `01-failover-docker-kill-muto.mp4`; per la scena 4, `03-maggioranza-persa-muto.mp4`.
...
Ha anche un **aspetto diverso** dagli altri undici: quelli mostrano solo il testo, questo mostra
...
## Le sei scene singole

Da usare quando una slide deve mostrare **un pezzo solo**. Mancano la 2 e la 4 per il motivo detto
in apertura: i montaggi `01` e `03` *sono* già quelle scene.
...
```bash
make filmati            # tutti e undici i fabbricati, da capo
make filmati NOME=05-guasto-shard-nei-due-profili-muto   # uno solo
make filmati-elenco     # che cosa produrrebbe, senza produrlo
```

Il dodicesimo — `04-avvio-offline-muto.mp4` — non si rifà così: si rigira con
